...
import com.zehnai.student.ui.viewmodel.TopicDetailViewModel
import com.zehnai.student.ui.viewmodel.appViewModelFactory

@Composable
fun TopicDetailScreen(topicId: String, onChallenge: (String) -> Unit) {
    val vm: TopicDetailViewModel = viewModel(factory = appViewModelFactory { TopicDetailViewModel(topicId) })
    ...
    when (val s = state) {
        ...
        is UiState.Success -> LazyColumn(Modifier.fillMaxSize().padding(16.dp)) {
            item { Text("Topshiriqlar") }
            items(s.data) { ch ->
                Card(Modifier.padding(top = 8.dp).clickable { onChallenge(ch.id) }) {
                    Text(ch.title ?: ch.id, Modifier.padding(12.dp))
                }
            }
        }
    }
