...
export function ChallengesPage() {
  ...
  const fetchAllChallenges = useCallback(async () => {
    try {
      ...
      if (targetClasses.length > 0 && failedClasses === targetClasses.length) {
        throw new Error('Unable to load challenges from the server')
      }
      ...
      setChallenges([])
      setLoadError(err.message || 'Unable to load challenges')
    } finally {
      setLoading(false)
    }
