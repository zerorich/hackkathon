...
ROLE_REGISTRY: dict[AgentRole, str] = {
    AgentRole.PLANNER: (
        "Plans challenge structure: focus areas, question type mix, batching. "
        "Output JSON only."
    ),
    AgentRole.RESEARCHER: (
        "Extracts learning objectives and key concepts from topic context. "
        "Output JSON only."
    ),
    ...
    AgentRole.CRITIC: (
        "Validates generated questions against schema and pedagogy rules."
    ),
    AgentRole.SYNTHESIZER: (
        "Merges validated questions into a cohesive challenge with title. "
        "Output JSON only."
    ),
}
